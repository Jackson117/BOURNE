...
if __name__ == '__main__':
    ...
    hypergraph_encoder_2 = copy.deepcopy(hypergraph_encoder)
    ...
    def eval(b):
        ...
        with torch.no_grad():
            ...
            dist_edge = args.alpha * dist_1 + args.beta * dist_2
            # dist_edge = args.alpha * (dist_1 + dist_3) / 2 + args.beta * (dist_2 + dist_4) / 2
            ano_score_e = dist_edge.cpu().numpy()

        scaler_e = MinMaxScaler()
        ano_score_e = scaler_e.fit_transform(ano_score_e.reshape(-1, 1)).reshape(-1)
        labels_e = b.ano_e_label[b.target_edge]
        return ano_score_e, labels_e.cpu().numpy()
    ...
    def save_model(epoch):
        ...
        torch.save(saved_content, 'checkpoint/{}/model_EdgePred_{}.pth'.format(args.dataset, args.epochs))
    ...
    def load_model(filename):
        ...
        loaded_content = torch.load('checkpoint/{}/model_EdgePred_{}.pth'.format(args.dataset, filename),
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        l_ano_score_e, l_ano_label_e, l_loss = [], [], []
        for b in data_loader:
            ...
            if epoch % args.eval_epochs == 0 or epoch == args.epochs:
                ano_score_e, ano_label_e = eval(b)
                l_ano_score_e.append(ano_score_e)
                l_ano_label_e.append(ano_label_e)
        if epoch % args.eval_epochs == 0 or epoch == args.epochs:
            loss_avg = np.mean(l_loss)
            all_ano_score_e = np.concatenate(l_ano_score_e, axis=0)
            all_ano_label_e = np.concatenate(l_ano_label_e, axis=0)
            recall, macro_f1, AUC, acc, precision = accuracy(all_ano_score_e, all_ano_label_e)
            if AUC > best_auc:
                best_auc = AUC
                save_model(epoch)
    end_tr = time.time()
    ...
    for i in tqdm(range(1, args.eval_rounds + 1)):
        ...
        if AUC > auc_max:
            saved_dict = {'ano_score': all_ano_score_e, 'ano_label': all_ano_label_e}
